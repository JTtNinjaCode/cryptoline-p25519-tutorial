...
3. 改 `shld $0x1, %r11, %rcx`, 轉成 `split r11_top r11_mid r11 63; shls bit_lost rcx_dbl rcx 1; adds add_co rcx rcx_dbl r11_top;`
...
8. `and r11 r11 0x7fff...` 保留, 後面加 `assert + assume r11 = r11_mid` bridge (借 step 3 split 的 r11_mid, 把 bitwise 結果橋給 Singular)
...
5. `and r10 r10 0x7fff...` 保留, 後面加 `assert + assume r10 = dc` bridge (借 step 2 already-existing 的 `split r11 dc r11 0x3f` 輸出, 把 bitwise 結果橋給 Singular)
